...
- **화자를 "바꾸는" 요구가 오면 강제 가능한 층은 하나뿐이다 — 모델에 도달하는 문자열 집합**(시스템 프롬프트 상시 블록 + 주입 지시들). 서버가 모델 출력을 거르는 지점(오디오 고정), 클라가 특정 화자만 음소거하는 지점(단일 스트림·화자 식별자 없음)은 **존재하지 않는다**. 그래서 "퇴장·복귀 금지"는 **1회성 턴 주입이 아니라 시스템 프롬프트 상시 블록**에 둬야 한다(매 턴 선두 = 이 구조의 최대 지속성).
- **경로 비대칭(자주 잊힌다)**: 폴백(텍스트)은 매 턴 `buildSystemPrompt`를 재조립해 상시 규칙이 **매 턴 재삽입**되지만, Gemini Live는 **토큰 발급 시 1회 고정**이다 → **주 경로가 구조적으로 더 약하다.** 설계 보고에 이 비대칭을 반드시 적을 것(T110/§22.2).

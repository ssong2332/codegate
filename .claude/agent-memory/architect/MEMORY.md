...
- [문면은 방출되는 모든 상태에서 참이어야 한다](feedback_wording_must_hold_in_every_emitting_state.md) — status 하나가 두 상태를 덮는다; 판정 함수 진리표를 열고 화자별로 한 행씩
- [조기 응답은 계약의 반쪽을 잃는다](feedback_client_shortcut_loses_the_contract_half.md) — 서버 우회 경로엔 서버 소유 문면이 없다; "다시 부르면 된다"는 멱등성부터, 해는 미리 내려보내기

...
- **쓰는 쪽에도 같은 규칙을 걸어라(§65 빈 약속 지표)**: 새 지표를 설계할 때 *"결함일 때만 찍는다"* 로 하면 **내가 방금 이 함정을 다시 만드는 것**이다. **적용 대상 세션이면 값이 `false`여도 찍어라** — 방출된 라인 수 자체가 분모다. 대신 **적용 대상이 아니면 0줄**(분모 오염 금지)이라는 반대 조건을 같은 블록에 명문화한다.

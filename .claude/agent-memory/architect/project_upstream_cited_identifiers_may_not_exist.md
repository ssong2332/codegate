...
⭐ **인계가 지목한 *문서 절 번호*도 같다.** OQ-A75 패스(2026-09-16)에서 인계문이 *"`docs/Architecture.md` §63을
읽어라 — 거기에 정정이 있다"* 고 했으나 **`§63` 문자열이 `docs/**` 전수에서 0히트**였다(최대 절 = §62, base
`716b920` 일치). ⇒ 착수 전 `^## ` 전수 grep은 **번호 확보용이자 인용처 실재 확인용**이다. 실재하지 않으면
그 절을 **내가 쓰는 것**이고, 인계된 *주장*은 별도로 코드에서 재판정해야 한다(그 건에서는 주장 자체는 참이었다).

...
- ⭐⭐ **"사용자 확정이 방금 내려왔다"는 지시문에서도 마찬가지다** — 오히려 여기가 더 위험하다. 2026-07-29 OQ-46 집행 패스에서 지시받은 판정 6건 중 **4건(대상 시나리오 · 완료 화면 형태 · 금액 표기 충돌 · 앵커 순서 제약)이 전날 병합된 §45에 이미 답이 있었다.** 사용자 확정문은 **OQ가 열려 있던 시점의 문면**이라, 그 사이 들어온 절이 이미 절반을 풀어 놨을 수 있다. ⇒ **확정 항목마다 "선행 절이 이 항목을 판정했는가" 열을 가진 표를 절 서두에** 두고, **일치하면 "확인만 한다"로 닫아라**(재판정 0건). 이 표가 이 패스 산출의 절반이었다.

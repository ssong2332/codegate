...
- [AC 번호 예약 금지](project_ac_number_reservation.md) — 미리 못박은 AC/T 번호는 반드시 소진된다(이 저장소에서 2회 발생)
- [AC·완료 증거 작성 형식](project_ac_verifiability.md) — 판정 불가 문장 금지, 역방향 확인·before/after 수치 필수, 추적성 갭 처리 절차
- [편집만 하고 커밋 금지](feedback_edit_only_handoff.md) — 브랜치·커밋은 오케스트레이터 몫, 신규 행은 append만

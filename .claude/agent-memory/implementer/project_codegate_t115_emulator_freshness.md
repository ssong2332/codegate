...
냉/온에 따라 반대 결과가 나온다. 함수 **목록**만은 부모 기동 시각에 고정된다(축 ③은 영향 없음).

**해결(같은 세션 3차)**: 판정표 #10 — *기동 이후 기록된 빌드 중 현재 해시와 다른 것이 있으면 `FRESH`
대신 `UNKNOWN`*(`findIntermediateBuilds`). **OS로 워커 spawn 시각을 조회하는 더 정밀한 안을 버리고
이걸 고른 기준: 정밀도 < 순수 함수 층에서 테스트 가능성.** reviewer가 바로 앞 라운드에 "CLI 통합
표면은 무테스트"를 지적했기 때문에, 그 표면을 키우는 해법은 같은 지적을 재생산한다.

⚠️ **노이즈 판정을 반드시 함께 실측하라**(이 규칙은 상시로 뜨면 장치를 죽인다): 편집 없이 `test` 2회 +
`build` 1회 → 해시 동일 → 0건. 테스트 파일만 수정 → 해시 불변 → 0건. **제품 코드를 고쳤다 되돌린
경우에만** 떴다. "내용이 다른"이라는 조건과 해시 범위 축소가 함께여야 이 성질이 성립한다.

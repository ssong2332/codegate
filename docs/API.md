...
| **거절 문자열**(2026-09-06 정본 — `docs/Architecture.md` §59.6 갱신 블록) | `declineInstruction`은 **`functions/src/scenarios/verifyIntercept.ts`의 모듈 상수** 2종(`SMS_DECLINE_*`와 같은 자리·같은 형식). ⭐ **`status:"too_early"` ⇒ `VERIFY_DECLINE_TOO_EARLY`** = *"(아직 확인 부서로 연결해 드릴 단계가 아니다. 연결해 드리겠다고 말하지 말고, 지금 하던 이야기를 그대로 이어가라. 조금 뒤에 다시 시도해도 된다.)"* · ⭐ **`status:"already_announced"` ⇒ `VERIFY_DECLINE_ALREADY`** = *"(그 안내는 이미 전달했다. 새로 안내하지 말고, 연결해 드리겠다는 말도 다시 하지 말고, 지금 하던 이야기를 그대로 이어가라.)"* — ⛔ **재연결 제안을 금지하는 절이 정본의 일부다**: `already_announced`는 **`placedAt` 존재(전환 완료)** 상태에서도 나가고 그때 화자는 **확인창구의 다른 담당자**라, "이미 안내한 창구로 연결해 드리겠다"는 재권유가 **T118/R-1·`transferStateLine`과 어긋난다**(판정 근거 `functions/src/verifyIntercept/buildDoc.ts:90-98` + `functions/src/verifyIntercept/index.ts:241`). 시나리오별 저작 **0건**. |

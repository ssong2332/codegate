...
| ⭐ **§31 증분(T123 · AC-080)** | ① **요청 enum에 `"landing_submitted"` 1값 추가** ⇒ `{ sessionId, smsId, event: "opened" \| "link_tapped" \| "landing_submitted" }`. **필드 추가 0건** — ⛔ 참가자가 입력한 값(계좌번호·예금주명)을 담을 필드는 요청·응답 어디에도 **존재하지 않는다**(AC-045 구조적 금지, `Architecture.md` §31.2). ② **쓰기**: `sessions/{sid}/inCallSms/{smsId}.landingSubmittedAt`을 **최초 1회만** 세팅. ③ **서버 검증 1건 추가**: `event === "landing_submitted"` ⇒ 그 문서에 **`fakeLandingId`가 있어야 한다**(없으면 `failed-precondition` — 링크가 없는 문자에 제출이 있을 수 없다). 기존 순서(인증·소유 → `status==="active"` → 문서 존재)는 무변경. ④ ⭐ **이 값만 리포트 판정 입력이 된다** — 위 *"범위 한정"* 행의 *"이 값은 판정 입력이 아니다"* 는 **`opened`·`link_tapped` 2종에 대해 그대로 유효**하고(§15.6 G22 유지 · **AC-080 (b)** 가 재확인), **`landing_submitted`만 예외**로 `deceivedMoments`에 **1건 승격**된다(AC-080 (c)). 앵커는 그 문서가 이미 가진 `anchorScammerTurn`을 기존 `resolveAnchor`로 해결하며, ⛔ **승격은 `anchorResolved === true && anchorTurnIndex >= 0`일 때만**이다(**G135**). ⑤ **리포트 표시 이벤트 신설 0건**(`deriveSmsEvents` 무변경 — G17 이중 렌더 회피). |
...
| 안전 | 창구명·재연결 라벨은 **전부 서버 카탈로그의 모의값**(AC-033/AC-005)이고 `url`/`tel`/발신 대상 필드가 **스키마에 없다**(AC-019 구조적 금지). LLM이 번호·기관명을 생성하지 않는다. ⭐ **T110 갱신(2026-07-28 — §22.3 예약 델타)**: `displayNumber`는 **카탈로그에서 제거**돼 신규 문서·스냅샷에 실리지 않는다. 그 필드가 지키던 검사 3종(번호 형식 · `REAL_WORLD_FORBIDDEN` 부분 일치 금지 · url/tel 스킴)은 **삭제된 것이 아니라 카탈로그 전 필드 순회 게이트로 승격**돼 커버리지가 **오히려 넓어졌다**(§22.3 G86-a/b/c — 현행은 하드코딩 3필드만 봤다). 화면 소스에 `displayNumber` 참조 0건도 **G85-UI**가 단언한다(`src/lib/verifyintercept/verifyIntercept.test.ts:146`). |
...
| ⭐ **§31 증분(T123 · AC-080 · 경로 B = `entrySurface:"messenger-link"`)** | ① **요청 enum에 `"submitted"` 1값 추가** ⇒ `{ sessionId, landingId, event: "shown" \| "consented" \| "submitted" }`. **필드 추가 0건**(위 *"안전"* 행 그대로 — 입력값을 담을 자리가 없다). ② **서버 검증 ⑤ 추가**: `event === "submitted"` 는 **`kind !== "app-install"`** 일 때만 → `invalid-argument`. ⭐ **기존 ④의 정확한 거울**이다 — `consented`는 app-install 전용, `submitted`는 credential-form 전용이며 **두 값이 겹치지 않는다**. ③ **쓰기**: `submittedAt`을 **최초 1회만** 세팅. ④ ⛔ **`"consented"` 를 제출에 재사용하지 않는다** — 그 값의 현행 의미는 *"가짜 **권한 허용**에 응했다"* 로 소스에 고정돼 있고(`mockScreens/index.ts:74-76` · `shared/types.ts:294-296` · `scenarios/mockScreens.ts:87-88`), 재사용하면 ④ 가드를 **약화**해야 하며 `MOCK_INSTALL_CONSENT_INSTRUCTION`(아래 `sendMessage` 확장)이 계좌 제출에도 붙는 경로가 열린다. ⑤ **승격**: `submittedAt` 존재 ⇒ `deceivedMoments` **1건 추가**(AC-080 (c)). `shownAt`만 있는 항목은 **그대로 승격 대상이 아니다**(D-51 ③ · AC-080 (b) 무변경). ⑥ **`MockScreenTimelineEntry` 필드 추가 0건** — `consented`는 계속 *"권한 허용 응낙"* 만 뜻한다(§31.9 ux-design 인계 1건). |
...
| **Response 증분 ②**(2026-09-07 — `docs/Architecture.md` **§59.6 갱신 2** · **G394** · `docs/DECISIONS.md` **#98**) | `{ …liveTools, verifyAlreadyAnnouncedInstruction?: string }` — **옵셔널 문자열 1개 추가**(위 행의 객체 안, **`offerVerificationDesk` 바로 아래·`failureInstruction` 위**). Request **무변경**. ⛔ **부재 = 오늘 동작 100%.** 값은 ⭐ **서버 상수 `VERIFY_DECLINE_ALREADY`의 사본**이다(`functions/src/scenarios/verifyIntercept.ts` — `deliverVerifyOffer`의 `status:"already_announced"` 응답이 쓰는 것과 **같은 문자열**, 새 문면 저작 **0건**). ⛔ **리터럴을 `realtime/liveTools.ts`에 복사하지 말 것 — import한다.** |
| **부착 조건**(증분 ②) | ⭐ **`offerVerificationDesk`와 정확히 같은 술어**(`hasVerifyIntercept && difficultyLevel==="advanced" && verifySeriesFor()==="A"`), **같은 스프레드 안**에서 함께 붙는다 — 채우는 함수는 **`functions/src/realtime/liveTools.ts`의 `buildLiveToolNames()` 한 곳뿐**이고 `buildLiveToolDeclarations()`(토큰에 고정되는 도구 선언)는 **0줄 변경**이다. 회귀 게이트가 *"둘은 항상 함께 있거나 함께 없다"* 를 전 시나리오 × 전 난이도로 단언한다(**G394**). |
| **왜 필요한가**(증분 ②) | 클라는 verify announce의 **이중 발동을 막는 클레임 가드**에서 지면(백스톱 경로가 같은 announce를 요청 중) **서버를 부르지 않고** 도구 응답을 돌려줘야 한다. ⛔ **이 상태에서 서버를 부르는 것은 해가 아니다** — `resolveVerifyOfferPlan`은 `placed===false`인 한 **몇 번을 불러도 `includeInstruction:true`** 를 돌려주므로(`functions/src/verifyIntercept/buildDoc.ts:90-98`) 돌아오는 것은 `already_announced`가 아니라 **`announced` + `announceInstruction` = 두 번 안내**다. 그 경합 상태는 **문서를 남기지 않는 클라 전용 상태**(announce는 `persist:false`, **G391**)라 서버가 관측할 자리 자체가 없다 ⇒ **문자열을 미리 내려보내는 것이 유일한 해**이며 `failureInstruction`이 푼 것과 **같은 문제**다. |
| 안전(증분 ②) | 문면에 **카탈로그 고유값(`deskLabel`·`offerId`·기관명) 0건 · 숫자 0건 · 가로채기 기전 서술 0건**(AC-005/OQ-38) ⇒ ⭐ **통화 시작 시점에 미리 내려보내도 사전 유출이 아니다**(창구명이 담긴 `announceInstruction`은 **여전히** credentials에 실리지 않는다 — 오퍼 시점 서버 렌더 무변경, §16.1.5). 같은 응답이 오늘 이미 `failureInstruction`(동일 계열)을 싣고 있으므로 **새 노출 경로 0건 — 필드 1개 추가다.** AC-024/AC-060 **무접촉.** |
...
| **거절 문자열**(⭐ **2종의 정본 위치가 갈린다** — `VERIFY_DECLINE_TOO_EARLY`는 **2026-09-16 교체본**이 정본(`docs/Architecture.md` **§64.5** · `docs/DECISIONS.md` **#103**), `VERIFY_DECLINE_ALREADY`는 **종전 그대로**(`docs/Architecture.md` §59.6 갱신 블록 · `#97`)) | `declineInstruction`은 **`functions/src/scenarios/verifyIntercept.ts`의 모듈 상수** 2종(`SMS_DECLINE_*`와 같은 자리·같은 형식). ⭐ **`status:"too_early"` ⇒ `VERIFY_DECLINE_TOO_EARLY`** = *"(아직 확인 부서로 연결해 드릴 단계가 아니다. 연결해 주겠다는 말을 아직 하지 않았다면 하지 말고, 이미 그런 취지로 말했다면 그 말을 뒤집지도 더 확실히 약속하지도 말고, 지금 하던 이야기를 그대로 이어가라. 조금 뒤에 다시 시도해도 된다.)"* — ⛔ **상태별 양방향 금지가 정본의 일부다**(2026-09-16 교체 — 종전 원문 *"연결해 드리겠다고 말하지 말고"* 를 대체한다): 이 문자열이 나가는 지점은 **`functions/src/verifyIntercept/index.ts:249` 한 곳뿐**(`trigger:"model_tool"` + `stage:"announce"` 거절 · write 0회 · throw 없음)인데, `BLOCKING` 도구 호출은 **모델이 말을 마친 뒤/말하는 중에** 나가고 거절은 **그 뒤에** 도착한다 ⇒ 모델이 **참가자의 확인 의사에 이미 반응해 버린 상태**가 구조적으로 도달 가능하다. 옛 단방향 금지는 그 상태에서 **실행 불가능한 지시**가 되어 모델이 방금 한 말을 취소하는 자기모순을 낳았다(§62.1 L-1 라이브 관측). ⛔ **두 갈래 중 하나만 남기지 말 것** — *"뒤집지 마라"* 만 남기면 **더 확실한 약속**으로 밀고 나가고(빈 약속 심화), *"약속하지 마라"* 만 남기면 **취소**로 읽혀 같은 자기모순이 재현된다. 바이트 일치 + 2갈래 성분(`아직 하지 않았다면 하지 말고` / `이미 그런 취지로 말했다면`)은 **G404**가 단언한다(`functions/src/scenarios/__tests__/verifyIntercept.test.ts:1326-1341`). · ⭐ **`status:"already_announced"` ⇒ `VERIFY_DECLINE_ALREADY`** = *"(그 안내는 이미 전달했다. 새로 안내하지 말고, 연결해 드리겠다는 말도 다시 하지 말고, 지금 하던 이야기를 그대로 이어가라.)"* — ⛔ **재연결 제안을 금지하는 절이 정본의 일부다**: `already_announced`는 **`placedAt` 존재(전환 완료)** 상태에서도 나가고 그때 화자는 **확인창구의 다른 담당자**라, "이미 안내한 창구로 연결해 드리겠다"는 재권유가 **T118/R-1·`transferStateLine`과 어긋난다**(판정 근거 `functions/src/verifyIntercept/buildDoc.ts:90-98` + `functions/src/verifyIntercept/index.ts:241`). 시나리오별 저작 **0건**. |
| ⛔ 공개 콜러블 노출(정본 조건) | 이 콜러블은 **인증된 공개 콜러블**이라 UI를 거치지 않는 직접 호출로 `declineInstruction`이 응답에 실려 나간다. 그래서 두 문면은 **① 카탈로그 고유값(`deskLabel`·`offerId`·기관명) 0건 · ② 번호·계좌·금액 형태 0건 · ③ 가로채기 기전 서술 0건(AC-005/OQ-38) · ④ 같은 콜러블이 오늘 이미 돌려주는 `announceInstruction`(창구명 원문 포함)보다 엄격히 적은 정보**를 만족한다 ⇒ **새 유출 표면 0건.** ⚠️ 이 4조건은 **문면 자체가 유일한 보증**이다(이 상수들은 카탈로그 필드가 아니라 모듈 상수라 **G86-a/b/c 순회 밖**이다 — 기계 집행 없음, `docs/Architecture.md` §59.6 갱신 블록 자기 고지 (1)). |

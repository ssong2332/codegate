...
| **④** | **못 잡는 형태를 못 잡는다고 적기** | 남는 우회 형태를 **테스트 출력에 보이는 형태로** 남긴다: `[T108/한계] 아래 형태는 이 스캔이 잡지 못한다` 테스트를 두고, 그 샘플에서 위반이 **0건임을 `deepEqual([])`로 단언**한다(현행 대조군 `:174-175`와 같은 형태). 최소 대상 — 함수 반환값으로 만든 태그명(`const T = mk()`), 표현식이 낀 템플릿(`` `${a}뱅크` ``), `React.createElement("inp"+"ut")`. **①에서 ①안으로 강등됐다면 동적 태그 2종을 여기에 추가한다.** ⚠️ **이 테스트가 "통과"하는 것은 자랑이 아니라 고지다** — 주석에 그 취지를 적을 것 |

...
import { generateOpeningLine } from "../openingLine";
...
test("generateOpeningLine(): 유효한 scenarioId면 scammer 역할의 오프닝 대사를 반환한다(AC-003)", async () => {
  ...
  assert.equal(opening.message.role, "scammer");
  assert.ok(opening.message.text.length > 0);
});
...
// realtime/provider.ts의 동일 readSecret 패턴과 동일한 이유). isUsingMockLlm()이 제거되고
// generateOpeningLine()이 자신이 관측한 isMock을 직접 반환하도록 바뀐 뒤에도(openingLine.ts
// 헤더 주석 참고) 같은 사실을 이 필드로 확인할 수 있다.
test("generateOpeningLine(): Functions 런타임 secret 바인딩이 없는 단위 테스트 컨텍스트에서는 isMock이 항상 true다", async () => {
  const opening = await generateOpeningLine(FAMILY_ACCIDENT_SCENARIO_ID);
  assert.equal(opening.isMock, true);
});

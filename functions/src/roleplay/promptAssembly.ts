...
- **페이로드는 가상값만 쓴다.** 계좌번호·앱 이름·링크·연락처를 요구하거나 불러 줄 때는 실존하지 않는 가상의 값을 쓴다(예: "OO은행 000-0000-0000"). **요구하는 행위 자체는 그대로 하되**, 실제로 돈이 들어가거나 실제로 동작하는 값만 쓰지 않는다 — 참가자가 겪는 판단 상황은 동일하다.`;

...
const CATEGORY_RULES: ReadonlyArray<{ category: TacticCategory; pattern: RegExp }> = [
  ...
  },
  {
    ...
    //    ⚠️ T82(2026-07-26) "권한" 추가 — 축 E3(앱 설치·원격제어 요구) 계열 라벨 "권한 허용 유도"가
    //    `other`로 떨어졌다(오케스트레이터 실측). 원격제어는 설치 다음에 **권한을 허용시키는** 단계라
    //    참가자 관점에서 같은 학습 묶음이다. ⚠️ **"원격"은 추가하지 않았다** — "모의 앱 설치"·
    //    "원격제어 앱 설치"·"앱 설치 지시"는 이미 "앱"으로 매치된다(§15.10.9 G33의 지적은 부정확했다).
    category: "link_or_install",
    pattern: /링크|클릭|설치|앱|URL|권한|디지털\s*취약/,
  },
  {
    ...
    //    ⚠️ T82(2026-07-26) "확인 …무력화" 추가 — 축 D3(확인 시도 무력화) 계열 라벨 "확인 시도
    //    무력화"·"확인 무력화"가 `other`로 떨어졌다(오케스트레이터 실측). 기법은 정반대지만(막는다
    //    vs 권한다) 참가자 관점에서는 둘 다 **"확인이 무력화된 결과"** 로 같은 학습 묶음이다 —
    //    분리하면 "확인 관련해서 3번 넘어갔다"가 흩어져 §15.6 G14의 실패가 재발한다(OQ-U25 판정).
    //    "확인"에 붙여서만 매치한다 — "무력화" 단독으로 잡으면 무관한 라벨까지 삼킨다.
    category: "verification_block",
    pattern:
      /확인[^]{0,6}차단|차단[^]{0,6}확인|확인[^]{0,6}무력화|절차\s*차단|신고\s*차단|끊음\s*저지|비밀\s*유지|고립|얼버무|채널\s*전환/,
  },
  {
  ...
  },
  {
    ...
    //    ⚠️ T82(2026-07-26) "정당화" 추가 — 축 D4(절차·서류 정당화) 계열 라벨 "절차·서류 정당화"가
    //    `other`로 떨어졌다(오케스트레이터 실측). 이미 이 행에 있는 "권위·정당성 포장"(loanScam)과
    //    같은 묶음이다 — 둘 다 "공식처럼 보여서 넘어갔다"는 하나의 학습 지점이다.
    //    ⚠️ 순서 주의(G34): D4 라벨에 "계좌"·"이체"·"본인 확인"을 섞으면 1·2행이 먼저 먹는다.
    category: "authority",
    pattern: /권위|기관|수사|경찰|공공|정당성|정당화|제3자\s*통화/,
  },
  {

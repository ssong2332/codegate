...
export function deriveVerifyEvents(
  ...
  // ⭐⭐ **§55 D4(R-D) — 연결 문장 1개.** 사용자 신고: 전환 뒤의 대사가 리플레이에서 **원 사기범
  // 번호로 라벨링된다.** 그런데 그 라벨은 **사실이다** — 참가자는 어디에도 새로 걸지 않았고
  // (ADR-0013 호 전환), 전사의 발신자 표기는 *"이 통화가 누구에게서 왔나"* 이며 그것은 끝까지
  // 하나였다. ⛔ **그러므로 라벨을 데스크 이름으로 갈아끼우지 않는다(G349)** — ① 어느 말풍선부터가
  // 데스크인지 오늘 어느 층도 관측하지 않고(경계 미상) ② 두 번째 발신자 소스 금지(§22.4 C2·G87)
  // ③ 번호가 끝까지 하나였다는 **학습 포인트 자체를 지운다**.
  // ⇒ 빠진 것은 라벨이 아니라 **연결 문장**이었다(결함의 정확한 이름 = 표기 오류가 아니라 설명
  // 공백). 축은 ⓐ 이 뒤의 상대 대사는 넘겨받은 담당자의 말 ⓑ 참가자는 새로 걸지 않았다 둘뿐이다.
  // ⛔ **특정 말풍선을 지목하지 않는다**(경계 미상) · ⛔ `placedAt`이 있는 **이 이벤트 안에만**
  // 둔다(§38.6 S1이 고친 결함의 재발 경로 — G352) · ⛔ 화면 코드 0줄(§16.3.5).
  // ⚠️ **잠정 초안이다 — 확정 카피는 ux-design 소관**(OQ-A5·§55.7 (1) 선례).
  const transferContinuity =
    "참가자가 새로 전화를 건 것이 아니라 같은 통화가 그대로 이어졌고, 이 뒤로 이어지는 상대 대사는 넘겨받은 담당자의 말입니다.";
  events.push({
    event: "verify_reconnected",
    what:
      outcome === "placed_and_complied"
        ? `상대가 ${doc.deskLabel}로 통화를 넘겼습니다. ${transferContinuity} 넘겨받은 담당자가 같은 요구를 '확인해 드렸다'는 형태로 이어갔고, 그 요구에 응했습니다.`
        : `상대가 ${doc.deskLabel}로 통화를 넘겼습니다. ${transferContinuity} 넘겨받은 담당자의 요구에 응하지 않았습니다.`,

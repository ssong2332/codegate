...
// ══════════════════════════════════════════════════════════════════════════════
// ⭐ §59.6 갱신(2026-09-06) / §64.5·§64.8 G404(2026-09-16, D-2) — `deliverVerifyOffer` 거절
// 문자열 2종의 **정본 고정**.
//
// 이 두 상수는 카탈로그 필드가 아니라 모듈 상수라 위 G86-a/b/c(`Object.entries` 전 필드 순회)
// 밖에 있다(architect 자기 고지, `docs/Architecture.md` §59.6 갱신 블록 (1) / §64.6 승계). 정본은
// `VERIFY_DECLINE_TOO_EARLY` = `docs/Architecture.md` §64.5(`#103`), `VERIFY_DECLINE_ALREADY` =
// 종전 그대로 §59.6 갱신 블록과 `docs/API.md` 부록 C `deliverVerifyOffer` 증분 표(`#97`)이며,
// 이 테스트는 그 문면과 소스 값이 어긋나지 않는다는 것만 고정한다(문면 저작은 이 테스트의 소관이 아니다).
// ══════════════════════════════════════════════════════════════════════════════
test("[§59.6 갱신/§64.8 G404] VERIFY_DECLINE_TOO_EARLY/ALREADY가 정본 문면과 바이트 단위로 일치한다", () => {
  assert.equal(
    VERIFY_DECLINE_TOO_EARLY,
    ...
    "TOO_EARLY는 §64.5/#103 정본으로 교체됐다 — 한 글자도 바뀌면 안 된다",
  );
  // ⭐ G404 — 상태 2갈래 성분이 둘 다 존재해야 한다(§64.5 표). 바이트 일치가 조건절 삭제를 이미
  // 막지만, 이 두 단언은 "왜 두 갈래인가"를 실패 메시지에 실어 둔다.
  assert.ok(
    VERIFY_DECLINE_TOO_EARLY.includes("아직 하지 않았다면 하지 말고"),
    "G404: 상태 ①(아직 말하지 않음) 분기가 빠졌다 — §64.5 표의 첫 갈래",
  );
  assert.ok(
    VERIFY_DECLINE_TOO_EARLY.includes("이미 그런 취지로 말했다면"),
    "G404: 상태 ②(이미 반응함) 분기가 빠졌다 — §64.5 표의 둘째 갈래. 이 분기 없이는 " +
      "'말하지 말고'가 이미 말한 뒤에는 실행 불가능한 지시가 된다(§62.1 L-1 자기모순 재현)",
  );
  assert.equal(
    VERIFY_DECLINE_ALREADY,

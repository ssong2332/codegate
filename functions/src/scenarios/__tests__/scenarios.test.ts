 ...
 *
 * ⛔⛔ **인계 — 다음에 넓히기 전에 "몇 번째 확장인지"부터 세라(T112 reviewer 요구).**
 *
 *   **확장 카운트: 지금이 2번째다.** ① T109 = 활용형 대안 6개(최초) · ② T112 = 형태 슬롯 4개(이 표).
 *   다음 사람은 슬롯을 붙이기 전에 **이 줄의 숫자를 3으로 올리는 것인지 먼저 확인하고, 올린다면
 *   그 사실을 여기 적어라.**
 *
 *   **왜 세는가**: 어간+문법요소 분리는 활용형 통째 나열보다 일반화 수준이 한 단계 높지만,
 *   **여전히 유한한 문법 구성의 열거**다. 위 "못 잡는 형태" 6가지 중 하나가 9번째 표본으로 들어오면
 *   슬롯 ⑤·⑥이 또 붙는 구조 — **T110이 4라운드를 태운 자리의 완화판이지 근본 해법이 아니다.**
 *
 *   | 확장 횟수 | 그때 할 것 |
 *   |---|---|
 *   | 1~2번째 | 슬롯 추가로 대응한다(지금까지 온 경로) |
 *   | **3번째부터** | ⛔ **T110류 함정 재진입 신호로 본다.** 슬롯을 더 붙이기 전에 **구조 전환**을 검토할 것 — T110이 실제로 택한 해법은 *"패턴을 확정 문면 화이트리스트로 교체"* 였다. 검토 결과 슬롯 추가를 그대로 택하더라도 **왜 구조 전환이 아닌지 근거를 여기 남길 것.** |
 *
 *   ⚠️ 3번째라고 해서 자동으로 화이트리스트가 정답인 것은 아니다 — `docs/Architecture.md` §24.4가
 *   *"허용목록 역전"* 을 **오탐 위험으로 정량 기각**한 전례도 있다. 카운트는 **판단을 강제하는 장치**이지
 *   결론이 아니다.
 */
...
/**
 * ⛔ **단언이 아니다 — 감사 추적용 수치다(T112 reviewer 요구, T115에서 같은 형태가 지적된 재발분).**
 *
 * T112가 보고한 *"인용구 338종 / `factInquiry` 매치 8건"* 은 착수 당시 임시 프로브로 잰 값인데
 * 그 프로브를 지워서 **재확인할 방법이 없었다.** 그래서 수치만 남긴다.
 *
 * ⛔ **이 수치를 `assert`로 바꾸지 말 것.** 콘텐츠가 **정당한** 사실 조회를 늘리면 숫자가 오르는 것이
 * 정상이고, 단언으로 두면 그 순간 깨지는 **가짜 게이트**가 된다. 실질 회귀 방지는 위
 * `[T109/요구몰림]` 전수 스캔이 계속 한다 — 여기는 *"패턴을 넓혔더니 매치가 몇 건이 됐나"* 를
 * 다음 사람이 **커밋된 코드만으로 다시 재게** 해 주는 자리다.
 */
test("[T112/감사] 사실 조회 검출 규모를 출력한다(단언 아님 — 콘텐츠가 늘면 수치도 는다)", (t) => {
  const factInquiry = DEMAND_KINDS.find((d) => d.kind === "factInquiry");
  assert.ok(factInquiry, "factInquiry 종이 사라졌다 — 이 진단이 무엇을 세는지 알 수 없다.");
  const quotes = new Set(assembledPrompts().flatMap(({ text }) => quotedUtterances(text)));
  let hits = 0;
  const samples: string[] = [];
  for (const quote of quotes) {
    for (const match of quote.matchAll(new RegExp(factInquiry.pattern.source, "g"))) {
      hits += 1;
      samples.push(match[0]);
    }
  }
  t.diagnostic(
    `[T112/감사] 인용구 ${quotes.size}종 · factInquiry 매치 ${hits}건 · 슬롯 ` +
      `${FACT_INQUIRY_SLOTS.length}개 (T112 착수 시 실측: 338종 / 8건 / 슬롯 4개)`,
  );
  t.diagnostic(`[T112/감사] 매치 문자열: ${samples.join(" | ") || "없음"}`);
});

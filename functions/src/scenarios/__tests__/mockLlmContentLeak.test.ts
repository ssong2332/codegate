...
import { extractEscalationSignal } from "../../roleplay/escalationSignal";
...
test("MockLlmClient: 모든 시나리오의 weakenedTactics를 turn 0~N-1까지 순회해도 백틱·마커·저작지시 문구가 사용자 화면에 노출되지 않는다(AC-003, reviewer/QA 회귀 방지)", async () => {
  ...
  for (const [scenarioId, prompt] of Object.entries(SCENARIO_PROMPTS)) {
    ...
    for (let turn = 0; turn <= tactics.length; turn += 1) {
      ...
      // 실제 파이프라인(roleplay/index.ts)과 동일한 순서로 두 마커를 모두 제거한 뒤 검사한다 —
      // extractLinkMarker만 거치면 [[SIGNAL:...]]가 그대로 남아 대괄호 검사에 걸린다.
      const { text: linkFree } = extractLinkMarker(completion.text);
      const { text: cleaned } = extractEscalationSignal(linkFree);
...
test("MockLlmClient: weakenedTactics 원문에 [[LINK:...]]/[[SIGNAL:...]] 마커가 있는 시나리오는 0..N턴 중 최소 한 번은 그 마커가 실제로 결과에 살아남는다(포지티브 경로, reviewer/QA 재검증 반영)", async () => {
  ...
  for (const [scenarioId, prompt] of Object.entries(SCENARIO_PROMPTS)) {
    ...
    for (let turn = 0; turn <= tactics.length; turn += 1) {
      ...
      const { escalate } = extractEscalationSignal(linkFreeText);
      if (escalate) {
        seenMarkers.add("[[SIGNAL:ESCALATE_VOICE]]");
      }
    }

    for (const expected of expectedMarkers) {
      assert.ok(
        seenMarkers.has(expected),
        `${scenarioId}: weakenedTactics에 ${expected}가 정의돼 있는데 0..${tactics.length}턴 어디서도 실제로 살아남지 않았다 — extractTacticFlavor가 마커를 버리고 있을 수 있다.`,
      );
    }
  }
});

// 실사용 재현 회귀 방지(2026-07-24, 사용자 실측 리포트) — 위 두 테스트는 백틱·대괄호·특정
// 저작지시 어휘·마커 생존 여부만 검사해, "인용구가 아예 없는" weakenedTactics 항목이
// extractTacticFlavor()의 대시(—) 뒤 원문 전체를 그대로 대사로 흘려보내는(예: "왜 급전이
// 필요한지 구체적으로 물으면 답을 흐리며 화제를 돌린다"처럼 캐릭터가 자기 행동을 3인칭으로
// 해설하는 것처럼 들리는 문장) 이 정확한 버그 클래스를 못 잡았다(messenger-friend-loan-kakao
// 시나리오 실플레이에서 재현됨). extractTacticFlavor의 계약 자체("인용구가 있으면 그것만 쓰고,
// 없으면 대시 뒤 전체를 폴백으로 쓴다")를 우회하지 않고, 대신 "실제 콘텐츠는 전부 인용구를
// 갖춰야 한다"는 데이터 불변식을 직접 검증한다 — 폴백 경로 자체는 향후 콘텐츠를 위한 안전망으로
// 남겨 두되(mockClient.ts 주석 참고), 지금 존재하는 시나리오 중 그 안전망을 실제로 타는 것이
// 하나도 없어야 한다.
test("SCENARIO_PROMPTS: 모든 weakenedTactics 항목은 최소 1개의 '따옴표 인용구'를 포함한다(인용구 없으면 3인칭 지시문 원문이 그대로 대사로 샌다, 사용자 실측 회귀 방지)", () => {
  for (const [scenarioId, prompt] of Object.entries(SCENARIO_PROMPTS)) {
    prompt.weakenedTactics.forEach((tactic, idx) => {
      assert.match(
        tactic,
        /'[^']+'/,
        `${scenarioId}의 weakenedTactics[${idx}]에 따옴표 인용구가 없다 — MockLlmClient가 이 항목을 대사로 쓸 때 3인칭 지시문 원문("${tactic}")을 그대로 노출한다. 캐릭터가 실제로 말했을 법한 '따옴표 인용구'를 추가할 것.`,
      );
    });
  }
});

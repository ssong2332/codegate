...
//
// ⚠️ 아래 5종은 전부 `credential-form`이다(**신규 kind 0건** — D-58). 새로 붙는 화면이 모두
// "정보를 입력하게 만드는 화면"이라 기존 안전 계약(입력 허용·서버 미전송·외부 네비게이션 부재)과
// 정확히 일치하고, kind를 늘리면 안전 계약이 한 벌 더 늘어 AC-072가 금지한 "검증 경로 이중화"에
// 스스로 다가간다.
//
// ⚠️ **§51(사용자 라이브 신고) — `safe-account-transfer` 1종이 여기 더해졌다.** §45 ⓐ가 판정만
// 하고 구현이 0줄이던 이체형 랜딩(계좌번호·금액을 입력해 이체를 완료하는 화면)의 첫 집행이다.
// 나머지 이체형 2종(`card-relief-transfer`·`protect-account-transfer`)은 §51.5 판정에 따라
// 후속 커밋에서 더해진다 — 대상 판정·회피 방지(G319)의 근거는 `docs/Architecture.md` §51.
//
...
//   ④ 위 주석 표에 6개 landingId와 6개 미끼 인용이 전부 들어 있다(주석 ↔ 테스트 표 1:1)
 ...
 *
 * ⚠️ **§51**: `institutional-impersonation`이 여기 신규 등재된다(§45 ⓐ 집행 — 통화 경로
 * `credential-form` 이체형 랜딩 1건, `IN_CALL_SMS.institution-safe-account`가 병기한
 * `fakeLandingId`를 가리킨다). G159 트립와이어 — 이 시나리오의 도달 가능 랜딩은 **1건뿐**이다.
 */

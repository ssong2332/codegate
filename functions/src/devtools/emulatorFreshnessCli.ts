...
//   - ⚠️ **워커가 부모보다 나중 빌드를 물고 있는 경우** — **거짓 `FRESH`를 내던 갈래였고, 2026-07-28에
//     닫았다**(`findIntermediateBuilds`, 판정표 #10). 코드를 붙들고 있는 것은 부모가 아니라
//     `functionsEmulatorRuntime` **워커**이고 워커는 **첫 요청 때** 뜬다. 그래서 ⓐ 기동 → ⓑ 빌드(X)
//     → ⓒ 첫 호출(워커가 X를 물고 상주) → ⓓ 되돌려 빌드 순서가 되면 *기동 시점 해시 == 현재 해시* 인데도
//     워커는 X를 서빙한다. 실측: 부모 PID 56888(기동 04:49:35.435Z, hash b32587476ebf) / 워커
//     PID 65372(spawn 04:51:52.859Z, 직전 빌드 04:51:52.687Z hash 801e97fc1cd1) 상태에서 판정은
//     `FRESH`인데 응답은 `T115-STALE-PROBE ...` 였다. **지금은 그 상태에서 `UNKNOWN`이 나온다.**
//     **남는 것**: 이 규칙은 *"워커가 낡은 것을 물었다"* 를 확정하지 못하고 *"모른다"* 까지만 간다 —
//     실제로는 워커가 최신을 물고 있는데도 `UNKNOWN`이 나올 수 있다(보수적 방향의 과탐).
//     **더 정밀한 대안(미채택)**: 살아 있는 워커들의 spawn 시각을 OS로 조회해 기준선을 *부모 기동 시각
//     ∪ 워커 spawn 시각*으로 넓히면 정확한 `FRESH`/`STALE-CODE`를 낼 수 있다. 채택하지 않은 이유는
//     그 경로가 **바로 위에 적은 무테스트 통합 표면을 키우기** 때문이다. 정밀도가 필요해지면 그때 가라.
//   - 프로세스 기동 시각 조회는 Windows(`netstat`+CIM)와 POSIX(`lsof`+`ps`)만 지원한다.
//     실패하면 `UNKNOWN`이다.
//   - ⚠️ **이 파일의 통합 표면(`fetchBackends`·`listeningPid`·`processStartedAt`)에는 자동 테스트가
//     없다 — 수동 검증만 있다**(reviewer 지적, 2026-07-28). 라이브 에뮬레이터와 OS 도구가 있어야
//     돌아가기 때문이다. 즉 **`/backends` 응답 형태나 `netstat` 출력 형식이 바뀌면 `npm test`는
//     그것을 잡지 못하고**, 증상은 조용한 실패가 아니라 `UNKNOWN` 판정으로 나타난다. 판정표 자체
//     (`emulatorFreshness.ts`)는 순수 함수라 전부 테스트가 있다.

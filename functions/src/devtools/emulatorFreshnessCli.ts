...
//   - ⚠️ **워커가 부모보다 나중 빌드를 물고 있는 경우**(2026-07-28 실측). 코드를 실제로 붙들고 있는
//     것은 부모가 아니라 `functionsEmulatorRuntime` **워커**이고, 워커는 **첫 요청 때** 뜬다. 그래서
//     ⓐ 에뮬레이터 기동 → ⓑ 빌드(버전 X) → ⓒ 첫 호출(워커가 **X**를 물고 상주) → ⓓ 되돌려 빌드(해시가
//     기동 시점과 같아짐) 순서가 되면, 이 도구는 부모 기동 시각을 기준선으로 쓰므로 **`FRESH`라고
//     답하지만 살아 있는 워커는 여전히 X를 서빙한다.** 실측 증거: 부모 PID 56888(기동 04:49:35Z,
//     hash b32587476ebf) / 워커 PID 65372(spawn 04:51:52.859Z, 직전 빌드 기록 04:51:52.687Z
//     hash 801e97fc1cd1) 상태에서 판정은 `FRESH`인데 호출 응답은 `T115-STALE-PROBE ...`였다.
//     ⇒ **후속 제안(미구현, 별건 판단 필요)**: 기준선을 *부모 기동 시각* 하나가 아니라 *부모 기동 시각
//     ∪ 살아 있는 워커들의 spawn 시각*으로 넓히거나, OS 조회 없이 **기동 시각 이후의 빌드 기록 중 현재
//     해시와 다른 것이 하나라도 있으면 `FRESH` 대신 `UNKNOWN`** 으로 떨어뜨리면 닫힌다(둘 다 판정을
//     보수적으로만 움직인다). 이번 변경 범위 밖이라 **고치지 않고 고지만 한다.**
